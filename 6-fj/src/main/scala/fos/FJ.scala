...
class Triple extends Pair {
  Object thd;
  Triple(Object fst, Object snd, Object thd) {
    super(fst, snd); this.thd = thd; }
  Triple setthd(Object newthd) {
    return new Triple(this.fst, this.snd, newthd); }
  Pair getpair() {
    return new Pair(this.fst, this.snd); }
  Object test() {
    return (new Pair(this.fst, this.snd).fst); }
}
...
(new Triple(new A(), new B(), new Pair(new A(), new B()))).test()
